...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        bos_token: Optional[int] = None,
        mt_generate: Optional[bool] = None,
    ):
        ...
        with torch.autograd.profiler.record_function("EnsembleModel: forward_encoder"):
            if mt_generate:
                encoder_outs = self.model.forward_mt_encoder(net_input)
            else:
    ...
    def extract_encoder_features(
        ...
        if mt_generate:
            net_input = sample['net_text_input']
        else:
            net_input = sample["net_input"]

        with torch.autograd.profiler.record_function("EnsembleModel: forward_encoder"):
            if mt_generate:
                encoder_outs = self.model.get_mt_encoder_states(net_input)[0]
            else:
                encoder_outs = self.model.get_encoder_states(net_input)[0]
...
class EnsembleModel(nn.Module):
    ...
    def has_mt_encoder(self):
        return hasattr(self.single_model, "mt_encoder")
    ...
    def get_encoder_states(self, net_input: Dict[str, Tensor]):
        if not self.has_encoder():
            return None
        return [model.encoder.forward(net_input["src_tokens"],net_input["src_lengths"],return_all_hiddens=True) for model in self.models]
    def get_mt_encoder_states(self, net_input: Dict[str, Tensor]):
        if not self.has_mt_encoder():
            return None
        return [model.mt_encoder.forward(net_input["src_tokens"],net_input["src_lengths"],return_all_hiddens=True) for model in self.models]
    ...
    @torch.jit.export
    def forward_mt_encoder(self, net_input: Dict[str, Tensor]):
        if not self.has_mt_encoder():
            return None
        return [model.mt_encoder.forward_torchscript(net_input) for model in self.models]
