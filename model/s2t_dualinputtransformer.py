...
class DualInputEncoder(FairseqEncoder):
    ...
    @classmethod
    def build_spch_encoder(cls, args, task):
        cfg = {
            "input_feat_per_channel": args.input_feat_per_channel,
            "input_channels": args.input_channels,
            "subsampling_type": args.subsampling_type,
            "subsampling_layers": args.subsampling_layers,
            "subsampling_filter": args.subsampling_filter,
            "subsampling_kernel": args.subsampling_kernel,
            "subsampling_stride": args.subsampling_stride,
            "subsampling_norm": args.subsampling_norm,
            "subsampling_activation": args.subsampling_activation,
            "encoder_embed_dim": args.encoder_embed_dim,
            "encoder_ffn_embed_dim": args.encoder_ffn_embed_dim,
            "encoder_layers": args.sign_encoder_layers,
    ...
    def build_text_encoder(cls, args, src_dictionary, spch_encoder):
        if args.encoder_shared_layers > 0:
            mx_shared_layers = (
                args.sign_encoder_layers
                if args.sign_encoder_layers < args.text_encoder_layers
    ...
    def forward(
        ...
        **kwargs
        ...
        Args:
            ...
            src_lengths: tensor of original lengths of input utterances (sign) (B,)
        ...
        # src_tokens, src_lengths: sign only training
        # src_txt_tokens, src_txt_lengths: text only training
        # all valid: sign + text training
...
#    encoder: S2TTransformerEncoder for sign + TransformerEncoder for text
...
class DualInputS2TTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # encoder 1: S2TTransformerEncoder for sign
        parser.add_argument(
            "--conv-kernel-sizes",
            type=str,
            metavar="N",
        ...
        parser.add_argument(
            ...
            default=1.0,
            metavar="V",
            help="text input cost ratio relative to sign input cost",
        )
        parser.add_argument(
            "--init-scale",
            type=float,
        ...
        parser.add_argument(
            "--load-pretrain-sign-encoder",
            type=str,
            default="",
            metavar="EXPR",
            help=""" path to the pretrained sign encoder """,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--sign-encoder-adapter-type",
            type=str,
            metavar="EXPR",
            default="None",
            choices=["None", "Linear", "MLP"],
            help="add sign encoder adapter",
        )

    @classmethod
    def build_encoder(cls, args, task):
        spch_encoder = DualInputEncoder.build_spch_encoder(args, task)
        ...
        if args.load_pretrain_sign_encoder != "":
            if hasattr(spch_encoder, "encoder"):
                checkpoint_utils.load_pretrained_component_from_model(
                    spch_encoder.encoder, args.load_pretrain_sign_encoder
                )
            else:
                checkpoint_utils.load_pretrained_component_from_model(
                    spch_encoder, args.load_pretrain_sign_encoder
                )
        if (
            args.load_pretrain_text_encoder_last != ""
        ):  # if share encoder, sign encoder parameters will be used.
...
def dualinputs2ttransformer_base(args):
    ...
    args.add_sign_eos = getattr(args, "add_sign_eos", False)
