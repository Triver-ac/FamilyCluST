...
ST_SAVE_DIR=/home2/chu/fairseq_sign/sign/experiments/sp-10/multilingual/many2many/${INPUT_TYPE}-${SRC_LANGS}-${TGT_Langs}-kl
DATASET_DIR=/home2/chu/fairseq_sign/sign/dataset/sp-10/256x256
TENSORBOARD_DIR=/home2/chu/fairseq_sign/sign/experiments/tensorboard/many2many/${INPUT_TYPE}-${SRC_LANGS}-${TGT_Langs}-kl
rm -r $ST_SAVE_DIR
mkdir $ST_SAVE_DIR -p
...
nohup python /home2/chu/fairseq_sign/sign/train.py  $DATASET_DIR \
    ...
    --eval-mt-bleu --eval-bleu --eval-bleu-detok space --eval-bleu-remove-bpe sentencepiece \
    ...
    > ${ST_SAVE_DIR}/${INPUT_TYPE}-${SRC_LANGS}-${TGT_Langs}-kl.log 2>&1 &
    # --encoder-learned-pos \
    # --decoder-learned-pos \
